...
class Fanslist extends Common
{
    ...
    function index()
    {
        ...
        $res = FanslistService::indexList($this->apiFormatWhere($where), $field, $orderby, $limit, $page);
    ...
    function getuserimg()
    {
        ...
        $where['ifpic'] = 1;
        // $where['member_id'] = 20;
        $video = db('fanslist')->where($where)->find();
        if ($video) {
            ...
            $arr = db('fanslist')->where('fanslist_id', $video['fanslist_id'])->update($data);
            if ($arr) {
                echo '成功' . $pic;
            }
        } else {
